...
def run(actor, critic, n_iters, l,k):
    ...
    reward_list = []
    cur_reward_list = []
    n_iters=max(500,int(n_iters/pow(2,k)))
    file1 = open("./log_curScore_avgScore.txt","a+")
    ...
    for iter in range(n_iters):
        ...
        for i in count():
            ...
            if done:
                ...
                error = reward + alpha*entropy.detach() - value
                ...
                if np.remainder(iter,l) == 0:
                    value = critic(state)
                    next_value = critic(next_state)
                    advantage = reward + alpha*entropy.detach() - value
                ...
                error = reward + alpha*entropy.detach() + gamma * next_value.detach() - value
                critic_loss = error.pow(2)
                optimizerC.zero_grad()
                critic_loss.backward()
                optimizerC.step()
                ...
                if np.remainder(iter,l) == 0:
                    value = critic(state)
                    next_value = critic(next_state)
                    advantage = reward + alpha*entropy.detach() + gamma * next_value.detach() - value
                    actor_loss = -log_prob * advantage.detach()
                    optimizerA.zero_grad()
                    actor_loss.backward()
                    optimizerA.step()
    ...
    return np.std(reward_list), running_total_reward    
...
#         actor=actor, critic=critic, n_iters=n_iters
        ...
        for k in range(500):
            ...
            l_update_lr=0.0001           
            k_val.append(k)
            l_val.append(min(max(1,round(l)),200))
            l_prev=l
            if k%2==0:
            ...
            plt.savefig("./svg/ltraining_score_LunLan_A2C.svg")
            plt.savefig("./pdf/ltraining_score_LunLan_A2C.pdf")
            plt.savefig("./png/ltraining_score_LunLan_A2C.png")
            ...
            plt.savefig("./svg/ltraining_std_LunLan_A2C.svg")
            plt.savefig("./pdf/ltraining_std_LunLan_A2C.pdf")
            plt.savefig("./png/ltraining_std_LunLan_A2C.png")
            ...
            plt.savefig("./svg/l_value_LunLan_A2C.svg")
            plt.savefig("./pdf/l_value_LunLan_A2C.pdf")
            plt.savefig("./png/l_value_LunLan_A2C.png")
...
#     run(actor, critic, n_iters=iterations)
